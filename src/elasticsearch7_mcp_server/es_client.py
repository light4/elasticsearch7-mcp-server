...
try:
    ...
    warnings.warn("requests 模块不可用，RequestsElasticsearchClient 将不可用")
...
try:
    ...
    warnings.warn("opensearchpy 模块不可用，OpenSearch 客户端将不可用")


# Monkey patch Elasticsearch 版本检查
def _monkey_patch_elasticsearch():
    """
    修改 Elasticsearch 客户端内部函数以绕过兼容性检查
    """
    try:
        # 尝试找到并替换_get_meta_version 函数
        from elasticsearch.client import _get_path_components, _normalize_hosts
        ...
        warnings.warn(f"无法修改 Elasticsearch 客户端兼容性检查：{str(e)}")
        return False
...
# 自定义基于 requests 的简单客户端
if REQUESTS_AVAILABLE:

    class RequestsElasticsearchClient:
        """一个简单的基于 requests 的 Elasticsearch 客户端，没有兼容性检查"""
...
class ElasticsearchClient:
    def __init__(self, logger: logging.Logger):
        ...
        try:
            ...
            if self.es_client.ping():
                ...
                self.logger.info(
                    f"服务器版本：{info.get('version', {}).get('number', 'unknown')}"
                )
            else:
                self.logger.warning("无法 ping 通搜索服务器，但连接已建立")
    ...
    def _get_es_config(self):
        ...
        if not all([config["username"], config["password"]]):
            self.logger.warning(
                "Missing Elasticsearch credentials. Attempting to connect without authentication."
            )
